...
function buildPrepareFollowupAnswer(point,item,session){
    ...
    if(/产研团队|产研协作|需求被理解|研发理解|设计理解|协作时如何确保/.test(normalized)){
        return `在 ${section} 里，我会先把需求翻译成三个大家都能对齐的东西：目标用户是谁、要解决的具体问题是什么、上线后看什么指标。我不会只给一段抽象需求，而是会把关键场景、输入输出、异常情况和优先级写清楚，再拉产品、研发、设计一起过一遍。如果大家理解不一致，我会直接回到用户场景和验证指标上，比如先确认「${firstBullet}」到底想解决哪个节点的问题，再把「${secondBullet}」拆成可交付的版本。这样最后不是“大家感觉懂了”，而是每个人都知道为什么做、先做什么、做到什么算完成。`;
    }
    if(/竞品分析|关键洞察|竞品里看到什么|发现了什么/.test(normalized)){
        return `我做竞品分析时不会只抄功能清单，而是先看对方到底优先服务谁、把哪个流程做得最顺、以及它为了这个目标牺牲了什么。如果放回 ${section} 这段经历里，我会说我重点比较了竞品的核心路径、首次使用门槛和结果呈现方式，最后得到的洞察不是“别人功能更多”，而是他们把用户最想要的结果前置了，所以转化更顺。这个洞察会直接影响我的方案判断，比如我会优先强化「${firstBullet}」对应的主路径，而不是一开始就堆很多边缘功能。`;
    }
    if(/功能优先级|需求优先级|优先级/.test(normalized)){
        return `我当时不是按“谁声音大”来排功能，而是先看这个问题是不是高频出现、它卡的是不是主路径、以及我能不能在当前资源下快速验证。以 ${section} 这段经历来说，我会优先做「${firstBullet}」这一类能直接影响核心流程的事，因为它既能验证用户需求，也能为后续 ${leadMatch} 提供最直接的结果证据。剩下的优化项我会放到第二阶段，等核心链路跑通后再迭代。`;
    }
    if(/算法工程师|算法同学|模型同学|协作.*冲突|冲突.*解决/.test(normalized)){
        return `和算法同学协作时，我最先做的不是争论方案，而是先把目标定义成双方都认可的结果。产品更关心用户是不是真的会用，算法更关心效果、稳定性和实现成本，所以我会先把问题写成共同指标，再把方案拆成最小可验证版本。比如在 ${section} 这段经历里，我会先用「${firstBullet}」确认需求成立，再把「${secondBullet}」拆成准确率门槛、上线节奏和回滚条件。这样冲突就不会停在“这个方案好不好”，而是变成“先验证哪个假设、什么结果算通过”。`;
    }
    if(/漏斗分析|哪些指标|验证假设|如何验证/.test(normalized)){
        return `我不会只看一个留存数，而是先把链路拆成“进入 -> 首次完成关键动作 -> 继续使用 -> 次日/阶段留存”。放到 ${section} 这段经历里，我会重点看触达率、关键动作完成率、流失节点和次日留存，再把假设写成“具体是哪个环节出了问题，为什么”。像「${firstBullet}」这种场景，我会先只改一个最可能影响转化的点，再对比改前改后数据，避免一次改太多导致最后分不清到底什么起作用。`;
    }
    if(/重新设计|做什么改进|怎么改进|重做/.test(normalized)){
        return `如果让我重来一次，我不会先堆更多功能，而是优先补“可复用”和“可验证”这两层。第一，我会把「${firstBullet}」这部分抽成更标准的输入、规则和输出，减少以后换场景时重做；第二，我会在「${secondBullet}」对应的流程里提前埋好指标和反馈回路，这样每轮迭代都知道问题出在哪。这样改完后，它会更像一个能持续迭代的产品，而不只是一次性的项目。`;
    }
    if(/质量标准|可复用/.test(normalized)){
        return '我会把 Skill 的质量标准分成三层。第一层是结果对不对，比如输出是否稳定、错误率是否可控；第二层是过程稳不稳，比如输入异常时会不会崩、有没有兜底；第三层才是可复用性，也就是哪些规则、流程和接口可以迁到别的场景，而不是每次从头重写。';
    }
    if(/留存.*计算|指标变化|样本量|口径/.test(normalized)){
        return '我会先把口径说清楚，比如比较的是哪两个时间窗口、样本范围是什么、看的是次日还是阶段留存，再补一句有没有同步观察激活率、关键动作完成率或后续转化。这样面试官会觉得我不只是记住了一个数字，而是真的理解这个数字是怎么来的。';
    }
    if(/迁移到财务场景|财务场景/.test(normalized)){
        return `如果把这套能力迁到财务场景，我不会空讲“都可以做”，而是会先挑一个具体流程，比如报销审核或异常对账，再把输入、规则判断和输出动作讲清楚。${section} 这段经历里已经能证明我做过「${firstBullet}」这种判断和推进，所以迁移时重点就是把原来的用户问题替换成财务规则和风控要求。`;
    }
    if(/量化体验优化|客诉率|体验优化效果/.test(normalized)){
        return '体验优化不能只说“用户反馈更好了”，至少要补一个主指标和一个辅助指标。主指标可以是关键步骤成功率、完成时长或转化率，辅助指标可以是客诉率、人工介入率或重复咨询量，这样整段经历才像真实上线后的结果，而不是停留在感受层面。';
    }
    if(/风控规则|平衡安全与用户体验/.test(normalized)){
        return '我会先把必须拦截的高风险场景划出来，再设计分层拦截策略，而不是一刀切。真正的平衡点不在于“放宽还是收紧”，而在于哪些风险自动拦、哪些进入人工复核、哪些可以通过补充材料放行。这样既能守住底线，也不会把正常用户都挡在外面。';
    }
    if(/异常交易预警/.test(normalized)){
        return '异常交易预警我不会泛讲模型，而是会先讲监控哪些信号，比如金额异常、频次异常、账户关系异常，再讲触发阈值、分级响应和谁来处理。这样听起来才像真的做过业务流程设计，而不是只懂概念。';
    }
    if(/上线后用户反馈|迭代了哪些功能/.test(normalized)){
        return `上线后我最先看的不是“大家夸不夸”，而是用户在哪一步还卡住、哪些功能没被真正用起来。围绕「${firstBullet}」这条主线，我会优先迭代直接影响主路径的问题，而不是先做看起来高级但不影响结果的小优化。`;
    }
    return `在 ${section} 这段经历里，我会直接先讲「${firstBullet}」，因为这最能说明我不是只提想法，而是真的做了判断和推进。接着我会补「${secondBullet}」这一段，把我怎么拉齐团队、怎么推进落地、最后拿到了什么结果讲清楚。这样回答的重点不只是证明我做过事，而是证明我在 ${leadMatch} 这类要求上已经有可以迁移的方法和结果。`;
}
