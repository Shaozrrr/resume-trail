-- Merge guest and registered accounts when the same visitor upgrades to auth.
  ...
  if v_account.id is not null
    ...
    update public.rt_accounts
       set guest_id = v_guest_id,
           ...
           is_admin = coalesce(public.rt_accounts.is_admin, false)
             or coalesce(v_guest_account.is_admin, false)
             or v_first_admin,
           ...
           bonus_prepare_credits = coalesce(public.rt_accounts.bonus_prepare_credits, 0)
  ...
  if v_account.id is null then
    ...
    update public.rt_accounts
       set guest_id = case
           ...
           is_admin = case when public.rt_accounts.is_admin then true else v_first_admin end,
           last_seen_at = now()
