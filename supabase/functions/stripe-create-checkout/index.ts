...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!planKey || !accountId) {
      return json({ error: '支付参数不完整，请重新打开升级弹窗后再试。' }, 400)
    }
    ...
    await adminClient.from('rt_billing_orders').upsert(
      {
        ...
        metadata: {
          ...(session.metadata || {}),
          resolved_method_mode: resolvedMode,
        },
      },
    ...
    console.error('[stripe-create-checkout] unhandled error', message, error)
